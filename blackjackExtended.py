...
def dealPlayerCards(totalPlayerCards, win, deck, totalPlayerValue, totalToDraw, totalValueDealer, totalCardsDealer,
                    totalDealerToDraw, ace, aceToDraw, aceCardNumber, aceToDrawDealer, aceDealer, aceCardDealer, click):
...
def main():
    ...
    hitButton = Button(win, Point(650, 225), 100, 50, "Hit")
    hitButton.enable()

    # create the play again button
    playAgainButton = Button(win, Point(650, 325), 100, 50, "Play again")
    playAgainButton.enable()

    # create the quit button
    quitButton = Button(win, Point(650, 425), 100, 50, "Quit")
    quitButton.disable()

    click = Point(650, 325)
    while playAgainButton.clicked(click):
        ...
        #                           aceToDraw, aceCardNumber, aceToDrawDealer, aceDealer, aceCardDealer, click)
        # if whoWins == "Tie":
        ...
        # wait for the user to click the quit button before closing window
        click = win.getMouse()
        if quitButton.clicked(click):
            win.close()
        elif not playAgainButton.clicked(click):
            click = win.getMouse()
        else:
            totalDealerToDraw.setFill("white")
            totalToDraw.setFill("white")
